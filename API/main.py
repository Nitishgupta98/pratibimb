...
try:
    ...
    RAI_AVAILABLE = True
    logging.info("✅ RAI modules imported successfully")
except ImportError as e:
    RAI_AVAILABLE = False
    logging.warning(f"⚠️ RAI modules not available: {e}")
    ...
    try:
        ...
        if not request.visual_objects or len(request.visual_objects) == 0:
            ...
            return {
                "step": 10,
                "status": "success",
                "message": "⚠️ No visual objects available for ASCII art generation",
                "data": {
                    "ascii_art_content": "",
                    "ascii_results": {},
                    "art_blocks_count": 0,
        ...
        if not ascii_results:
            ...
            return {
                "step": 10,
                "status": "warning",
                "message": "⚠️ No ASCII art was generated. Check Gemini API configuration.",
                "data": {
                    "ascii_art_content": "",
                    "ascii_results": {},
                    "art_blocks_count": 0,
    ...
    try:
        ...
        if not request.visual_objects or len(request.visual_objects) == 0:
            ...
            return {
                "step": 11,
                "status": "success",
                "message": "⚠️ No visual objects available for Braille art generation",
                "data": {
                    "braille_art_content": "",
                    "ascii_results": {},
        ...
        if not ascii_results:
            ...
            return {
                "step": 11,
                "status": "warning",
                "message": "⚠️ No ASCII art available for Braille conversion",
                "data": {
                    "braille_art_content": "",
                    "ascii_results": {},
                    "conversion_completed": True
...
async def rai_content_analysis(request: RAIAnalysisRequest):
    ...
    try:
        ...
        if is_blocked or verdict == "UNSAFE":
            main_message = "🚫 Content blocked by Responsible AI analysis. Please review the detailed report."
            status_val = "blocked"
        elif verdict == "WARNING":
            main_message = f"⚠️ Responsible AI Content Analysis completed with warnings. Verdict: {verdict}."
            status_val = "success"
        else:
            main_message = f"✅ Responsible AI Content Analysis completed. Verdict: {verdict}."
            status_val = "success"
        ...
        return JSONResponse(
            ...
            content={
                "status": "error",
                "message": f"❌ RAI content analysis failed: {str(e)}",
                "timestamp": datetime.now().isoformat()
            }
        )
