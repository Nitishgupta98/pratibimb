...
def enrich_transcript_with_figures(
    ...
    if not enhanced_transcript:
        logging.error("Enhanced transcript is empty")
        return ""

    if not objects_json:
        logging.warning("Objects JSON is empty - returning transcript without figure tags")
    ...
    # If no objects, return transcript as-is
    if not objects or len(objects) == 0:
        logging.info("No visual objects to insert - returning transcript without figure tags")
        return enhanced_transcript
